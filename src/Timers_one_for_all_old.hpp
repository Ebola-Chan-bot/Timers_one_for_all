...
#define TOFA_AllocateTimerStuff                   \
	...
	template <typename _Rep, typename _Period>
	inline Exception DoAfter(std::function<void()>Do, std::chrono::duration<_Rep, _Period> After, uint8_t &Timer)
	{
		TOFA_AllocateTimerStuff;
	...
	template <typename _Rep, typename _Period>
	Exception PeriodicDoRepeat(std::chrono::duration<_Rep, _Period> Period, std::function<void()>Do, uint8_t &Timer, size_t Repeat = Advanced::TimerState::InfiniteRepeat, std::function<void()>DoneCallback = nullptr)
	{
		if (Repeat)
		{
			TOFA_AllocateTimerStuff;
