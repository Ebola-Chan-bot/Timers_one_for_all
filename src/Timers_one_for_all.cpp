...
TimerClass const* Timers_one_for_all::AllocateTimer() {
...
void TimerClass0::StartTiming() const {
	...
	_State.COMPA = &(_State.HandlerA = [&_State = _State]() {
		...
		_State.HandlerB = [&OverflowCount = _State.OverflowCountA]() { OverflowCount++; };
		OCR0A = 0;
		TIFR0 = -1;
...
void SystemTimerClass::StartTiming() const {
	...
	SystemState.HandlerB = []() { SystemState.OverflowCount++; };
...
void SystemTimerClass::Delay(Tick Time) const {
	...
	if (TimeTicks >> 24) {
		...
		if (volatile uint32_t OverflowCount = TimeTicks >> 24) {
			OverflowCount++;
			SystemState.Handler = &(SystemState.HandlerA = [&OverflowCount]() { if (!--OverflowCount)
				SysTick->CTRL = 0; });
	...
	SystemState.Handler = &(SystemState.HandlerA = []() { SysTick->CTRL = 0; });
	SysTick->LOAD = TimeTicks;
	SysTick->CTRL = CTRL;
	while (SysTick->CTRL & SysTick_CTRL_ENABLE_Msk)
		;
...
void RealTimerClass::StartTiming() const {
	...
	RealState.HandlerB = []() { RealState.OverflowCount++; };
	while (RTT->RTT_SR)
		;
	...
	_State.Handler = &(_State.HandlerA = [this]() {
		...
		_State.HandlerB = [this]() {
			if (++_State.OverflowCount == (F_CPU >> 7) / 32768) {
				++Channel.TC_CMR;
				_State.OverflowCount = 1;
				_State.Handler = &(_State.HandlerA = [&OverflowCount = _State.OverflowCount]() { OverflowCount++; });
			}
			};
